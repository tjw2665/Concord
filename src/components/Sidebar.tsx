...
export function Sidebar({
  ...
  return (
    <aside className="w-72 flex-shrink-0 bg-concord-bg-tertiary border-r border-[var(--border)] flex flex-col overflow-hidden">
      <div className="p-3 border-b border-[var(--border)]">
        <h2 className="text-sm font-semibold text-concord-text-primary">Connection</h2>
      </div>
      <div className="flex-1 overflow-y-auto p-3 space-y-3 text-xs font-mono">
        ...
        {/* ── Network Info (Wireshark-friendly) ─────────────────── */}
        <div className="border border-[var(--border)] rounded p-2 space-y-1.5 bg-concord-bg-primary">
          ...
          <div>
            ...
            {netStats?.listenPort && (
              <span className="text-concord-text-secondary ml-1 text-[10px]">
                (Wireshark: tcp.port == {netStats.listenPort})
              </span>
            )}
          </div>
          ...
          <div className="flex gap-3">
            <div>
              ...
              {(s?.sendFail ?? 0) > 0 && (
                <span className="text-red-400 ml-1">({s?.sendFail} fail)</span>
              )}
            </div>
            <div>
              ...
              {(s?.recvFail ?? 0) > 0 && (
                <span className="text-red-400 ml-1">({s?.recvFail} fail)</span>
              )}
            </div>
          </div>
              ...
              {netStats.connections.map((conn) => (
                ...
                >
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-concord-text-primary font-medium">
                      {conn.peerIdShort}...
                    </span>
                  </div>
                  <div className="text-concord-text-secondary break-all mt-0.5">
                    {conn.remoteAddr}
                  </div>
                  {conn.remotePort && (
                    <div className="text-concord-text-secondary">
                      Remote port: <span className="text-yellow-300">{conn.remotePort}</span>
                    </div>
                  )}
                  <div className="text-concord-text-secondary">
        ...
        <div>
          ...
          <p className="mt-0.5 text-concord-text-primary break-all">
            {myAddress ?? '(waiting)'}
          </p>
        </div>
        {lanAddress && (
        ...
        {sidecarLog && (
          <div>
            ...
            <pre className="mt-1 p-2 rounded bg-concord-bg-primary text-concord-text-secondary whitespace-pre-wrap break-words text-[10px] max-h-32 overflow-y-auto">
              {sidecarLog}
            </pre>
          </div>
        )}
        ...
        {debugLog.length > 0 && (
          <div>
            ...
            <pre className="mt-1 p-2 rounded bg-concord-bg-primary text-concord-text-secondary whitespace-pre-wrap break-words text-[10px] max-h-48 overflow-y-auto">
              {debugLog.join('\n')}
            </pre>
          </div>
        )}
